...
@Service
public class PreferredProductsSkinService extends RecommendService {

    @Autowired
    private UserRecommendTemplate tmp;
    public PreferredProductsSkinService () {
        serviceName = ServiceType.PREFERRED_PRODUCTS_BY_REVIEW_WITH_SKIN.getDesc();
    }
    @Override
    public RecommendDTO getRecommendDTO(RequestParam param) {
        ...
        if (param.getMemberId()!= null) {
            ...
            if (rec.getPreferredProducts() != null) {
                preferredProducts.addAll(rec.getPreferredProductsWithSkin());
            }
            /**
