 ...
 */
@Service
public class PreferredProductsStdCateService extends RecommendService {
    @Autowired
    private UserRecommendTemplate tmp;
    public PreferredProductsStdCateService() {
        serviceName = ServiceType.PREFERRED_PRODUCTS_BY_REVIEW_WITH_STD_CATE.getDesc();
    ...
    public RecommendDTO getRecommendDTO(RequestParam param) {
        ...
        if (param.getMemberId()!= null) {
            ...
            if (rec.getPreferredProducts() != null) {
                preferredProducts.addAll(rec.getPreferredProductsWithStdCate());
            }
            /**
