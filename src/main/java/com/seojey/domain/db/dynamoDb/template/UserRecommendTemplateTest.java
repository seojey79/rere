...
public class UserRecommendTemplateTest {

    static AmazonDynamoDBClient client = new AmazonDynamoDBClient(new ProfileCredentialsProvider());

    static List<Integer> productIDs = Lists.newArrayList(0, -1);

    UserRecommendTemplate template ;

    public static void main(String[] args) throws Exception {
        try {
            BasicAWSCredentials credentials = new BasicAWSCredentials("AKIAIAHNHJG3CYQ4YD5A", "FLjCFxR6UNCaL2AzuvBh+y43q/qups1TyJy0yLhU");
    ...
    private static void GetBook(DynamoDBMapper mapper, int id) throws Exception {
        ...
        Book book = mapper.load(Book.class, 0);

        System.out.format("Id = %s Title = %s, ISBN = %s %n", book.getId(), book.getTitle(), book.getISBN());


        UserRecommend rcmd = mapper.load(UserRecommend.class, 0);

        ArrayList<Object> itemsToGet = new ArrayList<Object>();
        UserRecommend test1 = new UserRecommend();
        test1.setMemberId(0);
        itemsToGet.add(test1);
        UserRecommend test2 = new UserRecommend();
        test2.setMemberId(-1);
        itemsToGet.add(test2);

        Map<String, List<Object>> batchRect = mapper.batchLoad(itemsToGet);
//        System.out.format("Id = %s all = %s, skin = %s , cate = %s%n", rcmd.getMemberId(),
//                rcmd.getPreferredProducts(), rcmd.getPreferredProductsWithStdCate(),
//                rcmd.getPreferredProductsWithSkin());
    }

    private static void getRcmd(DynamoDBMapper mapper, DynamoDB dynamoDB) throws Exception {
        UserRecommendTemplate template = new UserRecommendTemplate(mapper);

        List<UserRecommend> param = Lists.newArrayList();
        for (Integer productID : productIDs) {
            UserRecommend test = new UserRecommend();
            test.setMemberId(productID);
            param.add(test);
        }

        List<UserRecommend> ret = template.getsByKey(param);

        for (UserRecommend rcmd : ret) {
            System.out.format("Id = %s all = %s, skin = %s , cate = %s%n", rcmd.getMemberId(),
                    rcmd.getPreferredProducts(), rcmd.getPreferredProductsWithStdCate(),
                    rcmd.getPreferredProductsWithSkin());
        }
